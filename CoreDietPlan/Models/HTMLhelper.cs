...
namespace CoreDietPlan.Models
{
    public static class HTMLhelper
    {
        public static HtmlString FoodDiv(Meal items)
        {
            ...
            sb.Append($"  <div class=\"col-lg-3 col-md-3 col-sm-3 col-xs-12 selectedFood myDiv\" id={trim} style=\"cursor:pointer;margin-top:5%;\">");
            sb.Append($"                                                <div class=\"card card-product\" style=\"border:1px solid lightgrey;border-radius:3px;height:100%;width:100%;\">");
            sb.Append($"                                                    <span onclick =\"removeThis('{trim}')\"  id={trim} style=\"visibility:hidden;\" class=\"pull-right clickable\" data-effect=\"fadeOut\"><i class=\"fa fa-times\"></i></span>");
            ...
            sb.Append($"                                                        <p readonly class=\"text-center\" id=\"protein\">Protein:<span>{items.nprotein}</span>g</p>");
            sb.Append($"                                                        <p class=\"text-center\" id=\"carb\">Carbs:<span>{items.ncarb}</span>g</p>");
            sb.Append($"                                                        <p class=\"text-center\" id=\"fat\">Fat:<span>{items.nfat}</span>g</p>");
            sb.Append($"                                                        <p class=\"text-center\" id=\"cal\">Calorie:<span>{items.ncalorie}</span>Kcal</p>");
